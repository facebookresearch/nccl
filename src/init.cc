...
static ncclResult_t computeBuffSizes(struct ncclComm* comm) {
  ...
  // MNNVL support
  if (!comm->MNNVL && comm->nNodes > 1) comm->p2pChunkSize = NCCL_P2P_NET_CHUNKSIZE;
  else if (comm->MNNVL || ncclTopoPathAllNVLink(comm->topo)) comm->p2pChunkSize = NCCL_P2P_NET_CHUNKSIZE;
...
#include "cudawrap.h"
  ...
  // MNNVL support
  {
    ...
    comm->MNNVL = NCCL_MNNVL < 0 ? cliqueSize == comm->nRanks : NCCL_MNNVL;
  ...
  if (collNetSupport(comm)) {
    const char *collNetEnable = NCCL_COLLNET_ENABLE.c_str();
...
static ncclResult_t ncclCommInitRankFunc(struct ncclAsyncJob* job_) {
  ...
  INFO(NCCL_INIT,"comm %p rank %d nranks %d cudaDev %d nvmlDev %d busId %lx commId 0x%llx commHash %lx %s - Init START",
       comm, comm->rank, comm->nRanks, comm->cudaDev, comm->nvmlDev, comm->busId,
       (unsigned long long)hashUniqueId(job->commId), comm->commHash, job->parent ? "CommSplit" : "CommInitRank");
