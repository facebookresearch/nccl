...
static const ncclKernelMatch ncclKerns[1+ncclNumTypes+NCCL_NUM_FUNCTIONS*ncclNumDevRedOps*ncclNumTypes*NCCL_NUM_ALGORITHMS*NCCL_NUM_PROTOCOLS] = {
  ...
  #endif
  #if HAVE_FP8
    {/*fp8E4M3*/(void*)NCCL_KERN_NAME(SendRecv, RING, SIMPLE, Sum, int8_t), false},
    {/*fp8E5M2*/(void*)NCCL_KERN_NAME(SendRecv, RING, SIMPLE, Sum, int8_t), false},
  #endif
...
ncclResult_t ncclEnqueueCheck(struct ncclInfo* info) {
  ...
  INFO(NCCL_COLL,"%s: opCount %lx sendbuff %p recvbuff %p count %zi datatype %d op %d root %d comm %p commHash %lx commDesc %s [nranks=%d, localRanks=%d] stream %p",
        info->opName, info->comm->opCount, info->sendbuff, info->recvbuff, info->count,
        info->datatype, info->op, info->root, info->comm, info->comm->commHash, info->comm->config.commDesc, info->comm->nRanks, info->comm->localRanks, info->stream);
