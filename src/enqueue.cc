...
static ncclResult_t computeCollChunkInfo(struct ncclInfo* collInfo, size_t nBytes, int nChannels) {
  ...
  if (collInfo->algorithm == NCCL_ALGO_COLLNET_DIRECT) {
    ...
    // Use uint64_t so that concurrentOps*chunkSize*X does not overflow
    uint64_t concurrentOps = nChannels * collInfo->comm->channels[0].nvls.nHeads;
    int maxChunkSize = NCCL_NVLSTREE_MAX_CHUNKSIZE;
...
ncclResult_t ncclEnqueueCheck(struct ncclInfo* info) {
  ...
  INFO(NCCL_COLL,"%s: opCount %lx sendbuff %p recvbuff %p count %zi datatype %d op %d root %d comm %p commHash %lu [nranks=%d] stream %p",
        info->opName, info->comm->opCount, info->sendbuff, info->recvbuff, info->count,
        info->datatype, info->op, info->root, info->comm, info->comm->commHash, info->comm->nRanks, info->stream);
