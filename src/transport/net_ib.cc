    ...
    if (ncclNIbDevs == -1) {
      ...
      for (int d=0; d<nIbDevs && ncclNIbDevs<MAX_IB_DEVS; d++) {
        ...
        for (int port_num = 1; port_num <= devAttr.phys_port_cnt; port_num++) {
          ...
          if (NCCL_IB_ADAPTIVE_ROUTING != NCCL_IB_ADAPTIVE_ROUTING_DEFAULT) ncclIbDevs[ncclNIbDevs].ar = NCCL_IB_ADAPTIVE_ROUTING;
...
ncclResult_t ncclIbInitCommDevBase(int ibDevN, struct ncclIbNetCommDevBase* base) {
  ...
  NCCLCHECK(wrap_ibv_create_cq(&base->cq, ibDev->context, 2*MAX_REQUESTS*NCCL_IB_QPS_PER_CONNECTION, NULL, NULL, 0));
  return ncclSuccess;
}
...
ib_connect_check:
  ...
  comm->base.nqps = NCCL_IB_QPS_PER_CONNECTION * comm->base.ndevs; // We must have at least 1 qp per-device
  ...
  for (int i = 0; i < comm->base.ndevs; i++) {
    ...
    if (devInfo->link_layer == IBV_LINK_LAYER_ETHERNET) {
      NCCLCHECK(wrap_ibv_query_gid(ibDev->context, ibDev->portNum, NCCL_IB_GID_INDEX, &commDev->base.gidInfo.localGid));
      ...
      for (int q = 0; q < comm->base.nqps; q++) {
        ...
        if (comm->base.qps[q].devIndex == i)
          INFO(NCCL_NET,"NET/IB: %s %d IbDev %d Port %d qpn %d mtu %d query_ece={supported=%d, vendor_id=0x%x, options=0x%x, comp_mask=0x%x} GID %ld (%lX/%lX) fifoRkey=0x%x fifoLkey=0x%x",
            ...
            commDev->base.ibDevN, ibDev->portNum, meta.qpInfo[q].qpn, devInfo->mtu, meta.qpInfo[q].ece_supported, meta.qpInfo[q].ece.vendor_id, meta.qpInfo[q].ece.options, meta.qpInfo[q].ece.comp_mask, NCCL_IB_GID_INDEX,
...
ib_recv:
  ...
  rComm->base.nqps  = NCCL_IB_QPS_PER_CONNECTION * rComm->base.ndevs; // We must have at least 1 qp per-device
  ...
  for (int i = 0; i < rComm->base.ndevs; i++) {
    ...
    NCCLCHECK(wrap_ibv_query_port(ibDev->context, ibDev->portNum, &ibDev->portAttr));
    NCCLCHECK(wrap_ibv_query_gid(ibDev->context, ibDev->portNum, NCCL_IB_GID_INDEX, &rCommDev->base.gidInfo.localGid));
  }
...
ncclResult_t ncclIbMultiSend(struct ncclIbSendComm* comm, int slot) {
  ...
  const int align = 128;
  int nqps = NCCL_IB_SPLIT_DATA_ON_QPS ? comm->base.nqps : comm->base.ndevs;
...
ncclResult_t ncclIbIsend(void* sendComm, void* data, int size, int tag, void* mhandle, void** request) {
  ...
  for (int r=0; r<nreqs; r++) {
    ...
    // Populate events
    int nEvents = NCCL_IB_SPLIT_DATA_ON_QPS ? comm->base.nqps : comm->base.ndevs;
    int qpIndex = comm->base.qpIndex;
    // Count down
    while (nEvents > 0) {
      ncclIbQp* qp = comm->base.qps + qpIndex;
...
ncclResult_t ncclIbIrecv(void* recvComm, int n, void** data, int* sizes, int* tags, void** mhandles, void** request) {
  ...
  const int nqps = NCCL_IB_SPLIT_DATA_ON_QPS ? comm->base.nqps : comm->base.ndevs;
    ...
    for (int i = 0; i < NCCL_IB_MAX_DEVS_PER_NIC; i++) {
      ...
      if (r->events[i]) {
        ...
        for (int w=0; w<wrDone; w++) {
          ...
          #ifdef ENABLE_TRACE
          char line[SOCKET_NAME_MAXLEN+1];
          TRACE(NCCL_NET, "Got completion from peer %s with status=%d opcode=%d len=%d wr_id=%lu r=%p type=%d events={%d,%d}, i=%d",
