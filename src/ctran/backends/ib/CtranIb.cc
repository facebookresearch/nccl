...
CtranIbSingleton::CtranIbSingleton(void) {
  ...
  if (!NCCL_IB_HCA_PREFIX.compare("=")) {
    ...
    for (int i = 0; i < nDevs; i++) {
      ...
      // Exclude: include only if it does not match with anyone in the excluding
      // list
      if (!NCCL_IB_HCA_PREFIX.compare("^")) {
        bool exclude = false;
        for (const auto& d : hcas) {
  ...
  if (devices.empty()) {
    WARN("CTRAN-IB : No active device found.");
    throw std::bad_alloc();
  }
...
CtranIb::CtranIb(ncclComm* comm) {
  ...
  if (foundPort) {
    INFO(
        NCCL_INIT,
        "CTRAN-IB: using device %s, port %d commHash %lu",
        s.devNames[comm->cudaDev].c_str(),
        this->pimpl_->port,
        comm->commHash);
  } else {
    WARN(
        "CTRAN-IB : No active port found on device %s. Disable IB backend.",
        s.devNames[comm->cudaDev].c_str());
  ...
  for (int r = 0; r < this->pimpl_->comm->nRanks; r++) {
    this->pimpl_->vcList.push_back(new CtranIb::Impl::VirtualConn(
        this->pimpl_->context,
        this->pimpl_->pd,
        this->pimpl_->cq,
        this->pimpl_->port,
  ...
  this->pimpl_->listenThread =
      std::thread{CtranIb::Impl::bootstrapAccept, this->pimpl_.get()};
  return;
}
...
ncclResult_t
CtranIb::regMem(const void* buf, std::size_t len, void** ibRegElem) {
  ncclResult_t res = ncclSuccess;

  int pageSize = getpagesize();
  if (len <= pageSize) {
    WARN(
        "CTRAN-IB: cannot register buffer, size (%lu) <= page size (%d)",
        len,
        pageSize);
    res = ncclSystemError;
    goto exit;
...
ncclResult_t CtranIb::progress(void) {
  ...
  while (1) {
    ...
    if (wc.status != IBV_WC_SUCCESS) {
      WARN(
          "CTRAN-IB: wrap_ibv_poll_cq failed, peerRank=%d, with status=%d, '%s'",
          vc->peerRank,
          wc.status,
          this->pimpl_->ibv_wc_status_str(wc.status));
      res = ncclSystemError;
      goto exit;
    }
  ...
  if (!this->pimpl_->pendingOps.empty()) {
    ...
    for (auto op : tmp) {
      int rank = (op->type == PendingOp::PendingOpType::ISEND_CTRL)
          ? op->isendCtrl.peerRank
       ...
       * order */
      if (std::find(peerRanks.begin(), peerRanks.end(), rank) !=
          peerRanks.end()) {
      ...
      if (op->type == PendingOp::PendingOpType::ISEND_CTRL) {
        ...
        if (vc->isReady() == true) {
          NCCLCHECKGOTO(
              vc->irecvCtrl(
                  op->irecvCtrl.buf, op->irecvCtrl.key, op->irecvCtrl.req),
              res,
              exit);
          delete op;
        } else {
...
ncclResult_t CtranIb::isendCtrl(
    void* buf,
    void* ibRegElem,
    int peerRank,
    CtranIbRequest** req) {
...
ncclResult_t CtranIb::irecvCtrl(
    void** buf,
...
ncclResult_t CtranIb::iput(
  ...
  NCCLCHECKGOTO(
      this->pimpl_->vcList[peerRank]->iput(
          sbuf, dbuf, len, ibRegElem, remoteAccessKey, notify, r),
      res,
      exit);
